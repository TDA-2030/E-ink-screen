EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 3
Title "epaper-show"
Date "2020-08-31"
Rev "V1..0"
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Device:C C1
U 1 1 5F4D2C09
P 3800 2900
F 0 "C1" H 3915 2946 50  0000 L CNN
F 1 "0.1uf" H 3915 2855 50  0000 L CNN
F 2 "Capacitor_SMD:C_0402_1005Metric" H 3838 2750 50  0001 C CNN
F 3 "~" H 3800 2900 50  0001 C CNN
	1    3800 2900
	1    0    0    -1  
$EndComp
$Comp
L Device:R R1
U 1 1 5F4D3040
P 3800 2600
F 0 "R1" H 3870 2646 50  0000 L CNN
F 1 "10K" H 3870 2555 50  0000 L CNN
F 2 "Resistor_SMD:R_0402_1005Metric" V 3730 2600 50  0001 C CNN
F 3 "~" H 3800 2600 50  0001 C CNN
	1    3800 2600
	1    0    0    -1  
$EndComp
Wire Wire Line
	4600 2750 3800 2750
Connection ~ 3800 2750
$Comp
L power:+3.3V #PWR0101
U 1 1 5F4D574F
P 3800 2450
F 0 "#PWR0101" H 3800 2300 50  0001 C CNN
F 1 "+3.3V" H 3815 2623 50  0000 C CNN
F 2 "" H 3800 2450 50  0001 C CNN
F 3 "" H 3800 2450 50  0001 C CNN
	1    3800 2450
	1    0    0    -1  
$EndComp
$Comp
L power:+3.3V #PWR0102
U 1 1 5F4D5EAB
P 5200 1950
F 0 "#PWR0102" H 5200 1800 50  0001 C CNN
F 1 "+3.3V" H 5215 2123 50  0000 C CNN
F 2 "" H 5200 1950 50  0001 C CNN
F 3 "" H 5200 1950 50  0001 C CNN
	1    5200 1950
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0103
U 1 1 5F4D6B4E
P 3800 3050
F 0 "#PWR0103" H 3800 2800 50  0001 C CNN
F 1 "GND" H 3805 2877 50  0000 C CNN
F 2 "" H 3800 3050 50  0001 C CNN
F 3 "" H 3800 3050 50  0001 C CNN
	1    3800 3050
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0104
U 1 1 5F4D7574
P 5200 5350
F 0 "#PWR0104" H 5200 5100 50  0001 C CNN
F 1 "GND" H 5205 5177 50  0000 C CNN
F 2 "" H 5200 5350 50  0001 C CNN
F 3 "" H 5200 5350 50  0001 C CNN
	1    5200 5350
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x06 J1
U 1 1 5F4D7FCD
P 1000 4800
F 0 "J1" H 1100 4300 50  0000 C CNN
F 1 "download" H 1150 4400 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x06_P2.54mm_Vertical" H 1000 4800 50  0001 C CNN
F 3 "~" H 1000 4800 50  0001 C CNN
	1    1000 4800
	-1   0    0    1   
$EndComp
Text Label 5950 2850 0    50   ~ 0
TXD
Text Label 5950 3050 0    50   ~ 0
RXD
Text Label 4350 2750 0    50   ~ 0
EN
Wire Wire Line
	5800 2750 5950 2750
Wire Wire Line
	5800 2850 5950 2850
Wire Wire Line
	5800 3050 5950 3050
Text Label 1200 4800 0    50   ~ 0
TXD
Text Label 1200 4900 0    50   ~ 0
RXD
Text Label 5950 2750 0    50   ~ 0
BOOT
Text Label 1200 4700 0    50   ~ 0
BOOT
Text Label 1200 4600 0    50   ~ 0
EN
$Comp
L power:GND #PWR0105
U 1 1 5F4DA4BE
P 1200 5000
F 0 "#PWR0105" H 1200 4750 50  0001 C CNN
F 1 "GND" H 1205 4827 50  0000 C CNN
F 2 "" H 1200 5000 50  0001 C CNN
F 3 "" H 1200 5000 50  0001 C CNN
	1    1200 5000
	1    0    0    -1  
$EndComp
$Comp
L power:+3.3V #PWR0106
U 1 1 5F4DAAEF
P 1200 4500
F 0 "#PWR0106" H 1200 4350 50  0001 C CNN
F 1 "+3.3V" H 1215 4673 50  0000 C CNN
F 2 "" H 1200 4500 50  0001 C CNN
F 3 "" H 1200 4500 50  0001 C CNN
	1    1200 4500
	1    0    0    -1  
$EndComp
$Comp
L Device:C C3
U 1 1 5F4DB89B
P 5050 2350
F 0 "C3" V 5100 2200 50  0000 L CNN
F 1 "10uf" V 5200 2250 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric" H 5088 2200 50  0001 C CNN
F 3 "~" H 5050 2350 50  0001 C CNN
	1    5050 2350
	0    1    1    0   
$EndComp
$Comp
L Device:C C2
U 1 1 5F4DDB9E
P 5050 2050
F 0 "C2" V 5100 1900 50  0000 L CNN
F 1 "1uf" V 5200 1950 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 5088 1900 50  0001 C CNN
F 3 "~" H 5050 2050 50  0001 C CNN
	1    5050 2050
	0    1    1    0   
$EndComp
Wire Wire Line
	5200 1950 5200 2050
Connection ~ 5200 2050
Wire Wire Line
	5200 2050 5200 2350
Connection ~ 5200 2350
Wire Wire Line
	5200 2350 5200 2550
Wire Wire Line
	4900 2050 4900 2350
$Comp
L power:GND #PWR0107
U 1 1 5F4E0168
P 4900 2350
F 0 "#PWR0107" H 4900 2100 50  0001 C CNN
F 1 "GND" H 4905 2177 50  0000 C CNN
F 2 "" H 4900 2350 50  0001 C CNN
F 3 "" H 4900 2350 50  0001 C CNN
	1    4900 2350
	0    1    1    0   
$EndComp
Connection ~ 4900 2350
$Comp
L Connector_Generic:Conn_01x09 J2
U 1 1 5F4E0C71
P 10600 3050
F 0 "J2" H 10680 3092 50  0000 L CNN
F 1 "epaper" H 10680 3001 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x09_P2.54mm_Horizontal" H 10600 3050 50  0001 C CNN
F 3 "~" H 10600 3050 50  0001 C CNN
	1    10600 3050
	1    0    0    -1  
$EndComp
Text Label 10100 2850 0    50   ~ 0
POWER
Text Label 10100 2950 0    50   ~ 0
RESET
Text Label 10100 3050 0    50   ~ 0
DC
Text Label 10100 3150 0    50   ~ 0
BUSY
Text Label 10100 3250 0    50   ~ 0
MOSI
Text Label 10100 3350 0    50   ~ 0
SCLK
Text Label 10100 3450 0    50   ~ 0
CS
$Comp
L power:+3.3V #PWR0108
U 1 1 5F4E377C
P 10400 2650
F 0 "#PWR0108" H 10400 2500 50  0001 C CNN
F 1 "+3.3V" H 10415 2823 50  0000 C CNN
F 2 "" H 10400 2650 50  0001 C CNN
F 3 "" H 10400 2650 50  0001 C CNN
	1    10400 2650
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0109
U 1 1 5F4E3F9A
P 10150 2650
F 0 "#PWR0109" H 10150 2400 50  0001 C CNN
F 1 "GND" H 10155 2477 50  0000 C CNN
F 2 "" H 10150 2650 50  0001 C CNN
F 3 "" H 10150 2650 50  0001 C CNN
	1    10150 2650
	-1   0    0    1   
$EndComp
Wire Wire Line
	10150 2650 10150 2750
Wire Wire Line
	10150 2750 10400 2750
Wire Wire Line
	10100 2850 10400 2850
Wire Wire Line
	10100 2950 10400 2950
Wire Wire Line
	10100 3050 10400 3050
Wire Wire Line
	10100 3150 10400 3150
Wire Wire Line
	10100 3250 10400 3250
Wire Wire Line
	10100 3350 10400 3350
Wire Wire Line
	10100 3450 10400 3450
Text Label 5950 3150 0    50   ~ 0
POWER
Text Label 5950 3250 0    50   ~ 0
RESET
Text Label 5950 3750 0    50   ~ 0
DC
Text Label 5950 3850 0    50   ~ 0
BUSY
Text Label 5950 3950 0    50   ~ 0
MOSI
Text Label 5950 4050 0    50   ~ 0
SCLK
Text Label 5950 4150 0    50   ~ 0
CS
Text Label 5950 3550 0    50   ~ 0
TMS
Text Label 5950 3450 0    50   ~ 0
TCK
Text Label 5950 3650 0    50   ~ 0
TDO
Text Label 5950 3350 0    50   ~ 0
TDI
Wire Wire Line
	5800 3350 5950 3350
Wire Wire Line
	5800 3450 5950 3450
Wire Wire Line
	5800 3550 5950 3550
Wire Wire Line
	5800 3650 5950 3650
Wire Wire Line
	5950 4150 5800 4150
Wire Wire Line
	5800 4050 5950 4050
Wire Wire Line
	5800 3950 5950 3950
Wire Wire Line
	5800 3850 5950 3850
Wire Wire Line
	5800 3750 5950 3750
Wire Wire Line
	5800 3150 5950 3150
Wire Wire Line
	5800 3250 5950 3250
$Sheet
S 6700 4650 1250 650 
U 5F4ECFED
F0 "Sheet5F4ECFED" 50
F1 "power.sch" 50
F2 "CHRG_STATE" O L 6700 4850 50 
F3 "BUTTON" O L 6700 4950 50 
F4 "ENABLE" I L 6700 4750 50 
F5 "USB_CHECK" O L 6700 5050 50 
F6 "ADC_VOL" O L 6700 5200 50 
$EndSheet
Wire Wire Line
	5800 4850 6700 4850
Wire Wire Line
	5800 4950 6700 4950
Wire Wire Line
	5800 5050 6700 5050
Wire Wire Line
	6700 4750 5800 4750
$Sheet
S 6700 3950 1250 450 
U 5F555EA8
F0 "Sheet5F555EA7" 50
F1 "speaker.sch" 50
F2 "SHUTDOWN" I L 6700 4150 50 
F3 "INPUT" I L 6700 4250 50 
$EndSheet
Wire Wire Line
	6700 4250 6450 4250
Wire Wire Line
	6350 4150 6700 4150
Wire Wire Line
	6350 4150 6350 4450
Wire Wire Line
	6350 4450 5800 4450
Wire Wire Line
	5800 4550 6450 4550
Wire Wire Line
	6450 4550 6450 4250
$Comp
L Device:LED D1
U 1 1 5F5659E0
P 7000 2950
F 0 "D1" H 6993 2695 50  0000 C CNN
F 1 "LED_BLUE" H 6993 2786 50  0000 C CNN
F 2 "LED_SMD:LED_0603_1608Metric" H 7000 2950 50  0001 C CNN
F 3 "~" H 7000 2950 50  0001 C CNN
	1    7000 2950
	-1   0    0    1   
$EndComp
Wire Wire Line
	6350 2950 5800 2950
$Comp
L power:GND #PWR0110
U 1 1 5F56B6A0
P 7150 2950
F 0 "#PWR0110" H 7150 2700 50  0001 C CNN
F 1 "GND" H 7155 2777 50  0000 C CNN
F 2 "" H 7150 2950 50  0001 C CNN
F 3 "" H 7150 2950 50  0001 C CNN
	1    7150 2950
	0    -1   -1   0   
$EndComp
$Comp
L Device:R R2
U 1 1 5F56C2A8
P 6500 2950
F 0 "R2" V 6600 2900 50  0000 L CNN
F 1 "4.7K" V 6400 2850 50  0000 L CNN
F 2 "Resistor_SMD:R_0402_1005Metric" V 6430 2950 50  0001 C CNN
F 3 "~" H 6500 2950 50  0001 C CNN
	1    6500 2950
	0    1    1    0   
$EndComp
Wire Wire Line
	6650 2950 6850 2950
NoConn ~ 4600 2950
NoConn ~ 4600 3950
NoConn ~ 4600 4050
NoConn ~ 4600 4150
NoConn ~ 4600 4250
NoConn ~ 4600 4350
NoConn ~ 4600 4450
$Comp
L Connector_Generic:Conn_01x05 J6
U 1 1 5F588EDB
P 1000 5800
F 0 "J6" H 918 5375 50  0000 C CNN
F 1 "JTAG" H 918 5466 50  0000 C CNN
F 2 "" H 1000 5800 50  0001 C CNN
F 3 "~" H 1000 5800 50  0001 C CNN
	1    1000 5800
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR01
U 1 1 5F589881
P 1200 6000
F 0 "#PWR01" H 1200 5750 50  0001 C CNN
F 1 "GND" H 1205 5827 50  0000 C CNN
F 2 "" H 1200 6000 50  0001 C CNN
F 3 "" H 1200 6000 50  0001 C CNN
	1    1200 6000
	1    0    0    -1  
$EndComp
Text Label 1200 5800 0    50   ~ 0
TMS
Text Label 1200 5700 0    50   ~ 0
TCK
Text Label 1200 5900 0    50   ~ 0
TDO
Text Label 1200 5600 0    50   ~ 0
TDI
NoConn ~ 5800 4250
NoConn ~ 5800 4350
NoConn ~ 5800 4650
Text Label 6300 5200 0    50   ~ 0
ADC_VOL
Text Label 4250 3050 0    50   ~ 0
ADC_VOL
$Comp
L RF_Module:ESP32-WROOM-32D U1
U 1 1 5F4D0EEA
P 5200 3950
F 0 "U1" H 4750 5300 50  0000 C CNN
F 1 "ESP32-WROOM-32D" H 5650 5350 50  0000 C CNN
F 2 "RF_Module:ESP32-WROOM-32" H 5200 2450 50  0001 C CNN
F 3 "https://www.espressif.com/sites/default/files/documentation/esp32-wroom-32d_esp32-wroom-32u_datasheet_en.pdf" H 4900 4000 50  0001 C CNN
	1    5200 3950
	1    0    0    -1  
$EndComp
Wire Wire Line
	4250 3050 4600 3050
Wire Wire Line
	6300 5200 6700 5200
$EndSCHEMATC
